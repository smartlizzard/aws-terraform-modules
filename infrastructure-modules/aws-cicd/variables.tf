...
variable "code_deploy_owner" {
  type        = string
  default     = ""
  description = "Code deploy owner. Ex. AWS, Custom and ThirdParty"
}

variable "code_deploy_provider" {
  type        = string
  default     = ""
  description = "Code deploy provider. Ex: CodeDeploy, ElasticBeanstalk, ECS"
}
...
variable "code_deployment_group_id" {
  type        = string
  default     = ""
...
variable "exiting_codepipeline_bucket" {
  type        = string
  default     = null
  description = "Whether want to use exiting s3 bucket for codepipeline"
}

variable "cf_action_mode" {
  type        = string
  default     = ""
  description = "Cloudformation action mode. Ex: CHANGE_SET_EXECUTE, CHANGE_SET_REPLACE, CREATE_UPDATE, DELETE_ONLY, REPLACE_ON_FAILURE. https://docs.aws.amazon.com/codepipeline/latest/userguide/action-reference-CloudFormation.html#action-reference-CloudFormation-type"
}
...
variable "cf_capabilities" {
  type        = string
  default     = ""
  description = "Cloudformation stack capability. Ex: CAPABILITY_IAM, CAPABILITY_NAMED_IAM, CAPABILITY_AUTO_EXPAND"
}

variable "cf_tamplate" {
  type        = string
  default     = ""
  description = "Cloudformation stack file name"
}

variable "deploy_cloudformation" {
  type        = bool
  default     = false
  description = "Use cloudformation for deployment"
}